...
class Agent {
  ...
  async callOllama(systemPrompt, userMessage) {
    try {
      ...
      // Handle HTTPS without cert verification if needed (for self-signed certs)
      if (this.config.baseURL.startsWith('https://')) {
        axiosConfig.httpsAgent = new (await import('https')).Agent({
      ...
      if (error.code === 'CERT_HAS_EXPIRED' || error.code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE') {
        throw new Error(
          `SSL Certificate error connecting to ${this.config.baseURL}. ` +
