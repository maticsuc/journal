...
class Agent {
  ...
  async callOllama(systemPrompt, userMessage) {
    try {
      ...
      // Handle HTTPS - Disable certificate verification for remote endpoints by default
      // This is needed for self-signed or improperly signed certificates
      // Set NODE_TLS_REJECT_UNAUTHORIZED=1 to re-enable verification
      if (this.config.baseURL.startsWith('https://')) {
        ...
        console.log(`[HTTPS Agent] baseURL=${this.config.baseURL}, rejectUnauthorized=${rejectUnauthorized}`);
      }
      
      console.log(`[Ollama Request] POST ${this.config.baseURL}/api/generate with model=${this.config.model}`);
      ...
      console.error('[Ollama Error Details]', {
        ...
        env_tls_reject: process.env.NODE_TLS_REJECT_UNAUTHORIZED,
      });
      ...
      if (error.code === 'CERT_HAS_EXPIRED' || error.code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE' || 
        ...
        throw new Error(
          ...
          `Try setting NODE_TLS_REJECT_UNAUTHORIZED=0 in production. ` +
          `Error: ${error.message}`
        );
      }
      throw error;
